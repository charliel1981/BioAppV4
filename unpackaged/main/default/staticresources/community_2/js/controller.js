  ...
  methods: {
    ...
    check: function () {
      setTimeout(function () {
        ...
        var width = el.getAttribute('width') || el.clientWidth;
        vm.position = 'calc(50% - ' + width / 2 + 'px)';
        vm.shown = 'block';
      }, 100);
    },
    // creates a click stat
    click: function (id) {
      vi.createStat(id)
    },
    // process html string
    content: function (content, clone, id) {
      ...
      if (content !== undefined) {
        ...
        if (process.split('<br>')) {
          ...
          var classname = clone == true ? 'content-clone' : 'content';
          element.setAttribute('class', classname);
          for (var a = 0; a < chunks.length; a++) {
            if (chunks[a].indexOf('<li>') !== -1) {
              var items = chunks[a].split('<li>');
              for (var b = 0; b < items.length; b++) {
                if (items[b].indexOf('</li>') !== -1) {
                  var el = document.createElement('li');
                  el.innerText = items[b].replace(/<\/li>/g, '');
                  element.appendChild(el);
                } else {
                  var el = document.createElement('span');
                  el.innerText = items[b];
                  element.appendChild(el);
                }
              }
            }
            if (chunks[a].replace(/\s/g, '').length !== 0 && chunks[a].indexOf('</a>') == -1 && chunks[a].indexOf('<li>') == -1) {
            ...
            if (chunks[a].indexOf('</a>') !== -1) {
              ...
              for (var b = 0; b < links.length; b++) {
                ...
                for (var c = 0; c < slices.length; c++) {
                  ...
                  if (slice.indexOf('href') == -1) {
                    var slip = slice.replace(/<u/g, '').replace(/<\/u/g, '').replace(/<\/a/g, '');
                    content += slip;
                  }
                  if (slice.indexOf('href') !== -1) {
              ...
              tag.setAttribute('onclick', '(function(e) { e.stopPropagation(); vm.click("' + id + '") })(event)')
      ...
      if (document.getElementById('toi')) {
        ...
        for (var a = 0; a < newlength; a++) {
          ...
          if (record != undefined) {
            ...
            container.setAttribute('onclick', 'vm.toggle(this), vm.click("' + record.recordId + '")');
            ...
            content.appendChild(vm.content(record.content, false, record.recordId));
            content.appendChild(vm.content(record.content, true, record.recordId));
    ...
    create_video: function (url) {
      ...
      if (url.indexOf('vimeo') != -1) {
        ...
        vimeo.setAttribute('webkitallowfullscreen', true);
        vimeo.setAttribute('mozallowfullscreen', true);
        vimeo.setAttribute('allowfullscreen', true);
        vimeo.setAttribute('id', 'video-viewer');
        var src = url.split('vimeo.com/')[1];
        if (src.indexOf('/') != -1) src = src.split('/')[0];
        vimeo.setAttribute('src', 'https://player.vimeo.com/video/' + src + '?autoplay=1');
        vimeo.setAttribute('width', 640);
        parent.appendChild(vimeo);
        vm.check();
      } else {
        var parent = document.getElementById('video-holder');
    ...
    run_global_search: function() {
      var search = encodeURIComponent(this.globalsearch);
      window.location.href = '/apex/CommunitySearchPage?search=' + search;
    },
    run_search: function(query) {
      ...
      if (query != null) vm.globalsearch = query;
      if (query == null) query = vm.globalsearch;
      query = '"' + query.replace(/"/g, '') + '"';
      document.getElementById('query').innerText = query;
      var newSearch = window.location.href.split('?search=')[0] + '?search=' + encodeURIComponent(rawQuery);
      window.history.pushState(null, 'Biocomposites Distributer Hub', newSearch);
      console.log(query);
      vi.runSearch(query, function(res) {
        var topics = {
          ...
          'e-learning': [],
          'Biocomposites internal documentation': [],
          'Video library': [],
          'Total': 0
        };
        for (var a = 0; a < res.length; a++) {
          try {
            var atype = res[a].Title__c || '';
            var atag = res[a].ALT_Tag__c || '';
            var amatch = (atype + atag).toLowerCase();
            if (amatch.indexOf(rawQuery) != -1) {
              var topic = res[a].RecordType.Name;
              if (topic == 'Marketing Resources') topic = 'Marketing collateral';
              if (topic == 'Left Panel' && res[a].Type__c == 'Marketing collateral') topic = 'Marketing collateral'; 
              if (topic == 'Left Panel' && res[a].Type__c == 'Account management') topic = 'Account management'; 
              if (topic == 'Left Panel' && res[a].Type__c == 'Sales tools') topic = 'Sales tools';
              if (res[a].Type__c == 'Sales tools') topic = 'Sales tools';
              if (topic == 'Left Panel' && res[a].Type__c == 'Internal only') topic = 'Biocomposites internal documentation';
              if (topic == 'Left Panel' && res[a].Type__c == 'Video box') topic = 'Video library';
              if (topic == 'Video Library') topic = 'Video library';
              if (topic == 'Continuing Education') topic = 'Continuing education';
              if (topic == 'Brainshark Training') topic = 'e-learning';
              var noop = ['Mobile App Permission', 'Publication Library', 'Topics Of Interest', 'Featured Item', 'Surgeon'];
              var link = null;
              if (res[a].Community_Item_Permissions_Links__r) {
                link = res[a].Community_Item_Permissions_Links__r[0].Button_Link_URL__c;
                if (link == null && res[a].Video_URL__c) link = res[a].Video_URL__c;
                if (link == null && res[a].Brainshark_URL__c) link = res[a].Brainshark_URL__c;
                res[a].Link = decodeURIComponent(link).replace(/&amp;/g, '&');
                if (noop.indexOf(topic) == -1 && link != null) {
                  topics[topic].push(res[a]);
                  topics.Total ++;
                }
              }
            }
          } catch(e) {
          console.error('Couldnt add result:', res[a], e);
          }
        }
        console.log(topics);
        vm.search_results = topics;
        console.log('results are: ', vm.search_results);
        vm.processed = true;
        vm.loading = false;
      });
    }
