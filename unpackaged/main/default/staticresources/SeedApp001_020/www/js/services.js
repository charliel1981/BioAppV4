...
(function() {
  ...
  function ProjectService($rootScope, $q, _, devUtils, SyncService, NotificationService, UserService) {
	  ...
	  var tablesToSync = ['MC_Project_Location__ap', 'MC_Time_Expense__ap' , 'MC_Phone_1__c', 'MC_Phone_1__c'];
	  ...
	  return {
	    ...
	    newOrder: function(varNewExp, success, error) {
	    	console.log("Inserting new order: " + JSON.stringify(varNewExp));
	      devUtils.insertRecord('MC_Project_Location__ap',varNewExp).then(function(res) {
	        success(res);
	        // perform background sync
	        SyncService.syncTables(['MC_Project_Location__ap'], true);
	      }).catch(function(e) {
	        error(e);
	      });
	    },
	              ...
	              if (typeof(resObject.status) != "undefined" && resObject.status != "100400") {
	                ...
	                UserService.hasDoneProcess("initialDataLoaded").then(function(result) {
	                  if (result) {
	                    ...
	                    SyncService.syncTables(tablesToSync, true);
	                  } else {
	                    ...
	                    SyncService.initialSync(tablesToSync);
	  ...
	  function getLocationFromId(locationId) {
	    ...
	    var location =  _.where($rootScope.locations, {'Id': locationId});
	    if (typeof location[0]!= 'undefined') {
	      console.log('Angular: location->' + location[0]);
	      return location[0].Name;
	    } else {
	      //console.log('Angular: no location yet');
	      return '-';
