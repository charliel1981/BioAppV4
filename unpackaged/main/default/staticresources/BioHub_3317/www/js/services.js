...
(function() {
  ...
  function DeployService($rootScope, $q, $timeout, $http) {
	  ...
	  function doesBundleExist(appConfig){
	    return new Promise(function(resolve, reject) {
			// TMP
			// Use empty record line for new DV
			resolve({records:[]});
	  ...
	  function uploadSWPage(appConfig) {
	    return new Promise(function(resolve, reject) {
	      $timeout(function() {
	        $http.get('../apex-templates/swpage-template.apex').success(function(data) {
	          var dataName = appConfig.sf_app_name + 'SW_' + appConfig.sf_app_vsn;
	          var cacheEntriesStr = '';
	          _.each($rootScope.deployFiles, function(el){
	            if (!el.dir) cacheEntriesStr += '"{!URLFOR($Resource.' + appConfig.sf_app_name + '_' + appConfig.sf_app_vsn + ', \'' + el.name + '\')}",\n';
	          });
	          ...
	          delete $rootScope.deployFiles;
						...
						var pageOptions = JSON.stringify({
							...
							"pageContents":dataParsed,
							"apiVersion":apiVersionInt,
							"pageDescription":"MobileCaddy Service Worker" });
	          force.request(
	            {
	              method: 'POST',
								path:"/services/apexrest/mobilecaddy1/PlatformDevUtilsR001",
	            ...
	            function(response) {
	            	...
	            	var respJson = JSON.parse(response);
	            	if (respJson.errorMessage == "success") {
	              	resolve('Service Worker uploaded');
	              } else {
		            	respJson.message = respJson.errorMessage;
		            	respJson.type = "error";
	              	console.error(respJson);
	            ...
	            function(error) {
	              console.error(error);
	              reject({message :'Service Worker page upload failed. See console for details.', type: 'error'});
	            }
	          );
    			});
	      }, 30);
	    ...
	    return new Promise(function(resolve, reject) {
	      $timeout(function() {
	        $http.get('../apex-templates/startpage-template.apex').success(function(data) {
	          ...
	          dataParsed = dataParsed.replace(/MY_SW_RESOURCE/g, appConfig.sf_app_name + 'SW_' + appConfig.sf_app_vsn);
    ...
    function getMenuItems() {
      return new Promise(function(resolve, reject) {
        devUtils.readRecords('Mobile_Refresh__ap', []).then(function(resObject) {
          // console.log("bio resObject.records",resObject.records);
          var records = _.chain(resObject.records)
            .filter(function(el){
...
(function() {
  ...
  function NetworkService($rootScope, SyncService, logger) {
    return {
      ...
      updateStateFromDevice: updateStateFromDevice
    };
    ...
    function getNetworkStatus() {
      try {
        ...
        logger.error("Error calling bootstrap.deviceIsOnline", e);
    ...
    function updateStateFromDevice() {
      try {
        var bootstrap = cordova.require("com.salesforce.util.bootstrap");
        console.log("bootstrap", bootstrap);
        if (bootstrap.deviceIsOnline()) {
...
(function() {
  ...
  SyncService.$inject = ['$rootScope', 'devUtils', 'logger', 'syncRefresh'];

  function SyncService($rootScope, devUtils, logger, syncRefresh) {
	  ...
	  function syncTables(tablesToSync, syncWithoutLocalUpdates, maxTableAge) {
	    ...
	    if (syncLock == "true") {
	      return;
			...
			// Case 6411 - Refresh token prior to each sync
			refreshAccessToken(
				function() {
					setSyncState("syncing");
	  ...
	  function setSyncLock(status) {
		...
		function refreshAccessToken(success, error) {
			if (window.LOCAL_DEV) {
				console.log ('Would have refreshed token');
				success();
			} else {
				try {
					syncRefresh.refreshToken(
						...
						function(error) {
							var getUrl = 'https://mc-logs.herokuapp.com/log?log="SyncTables: Token refresh error"';
							var xhr = new XMLHttpRequest();
							xhr.open('GET',getUrl, true);
							xhr.send();
							logger.error('SyncTables refreshToken', error);
							error();
					});
				} catch (e) {
					logger.error(e);
